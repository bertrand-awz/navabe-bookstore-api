...
def customer_welcome_email(user: User) -> EmailTemplate:
    ...
    text = (
        ...
        "Welcome to Navabe. Your customer account is ready.\n\n"
        f"Your Navabe identifier is {user.identifier}.\n\n"
        "Keep this identifier for sign-ins and account recovery.\n\n"
        "The Navabe team"
    )
    html = _layout(
        preheader=f"Your Navabe identifier is {user.identifier}.",
        title="Welcome to Navabe",
        greeting=f"Hello {user.first_name},",
        paragraphs=(
            "Your customer account is ready. You can now browse the catalog, "
            "place orders and manage your account with Navabe.",
            "Keep this identifier somewhere safe. It is used for sign-ins and "
            "account recovery.",
        ),
        details=(("Customer identifier", user.identifier),),
        footer_note="This email confirms the creation of your Navabe customer account.",
    )
    return EmailTemplate(subject, text, html)


def customer_password_changed_email(user: User) -> EmailTemplate:
    subject = "Navabe password changed"
    text = (
        f"Hello {user.first_name},\n\n"
        "Your Navabe password was changed.\n\n"
        "If you did not make this change, contact the Navabe team immediately.\n\n"
        "The Navabe team"
    )
    html = _layout(
        preheader="Your Navabe password was changed.",
        title="Password changed",
        greeting=f"Hello {user.first_name},",
        paragraphs=(
            "Your Navabe password was changed successfully.",
            "If you did not make this change, contact the Navabe team immediately.",
        ),
...
def customer_recovery_email(user: User, password: str) -> EmailTemplate:
    subject = "Navabe account recovery"
    text = (
        f"Hello {user.first_name},\n\n"
        "A temporary password was generated for your Navabe account.\n\n"
        f"Temporary password: {password}\n\n"
        "Use it to sign in, then change your password from your account settings.\n\n"
        "The Navabe team"
    )
    html = _layout(
        preheader="A temporary password was generated for your Navabe account.",
        title="Account recovery",
        greeting=f"Hello {user.first_name},",
        paragraphs=(
            "Use the temporary password below to sign in to your Navabe account.",
...
def admin_welcome_email(admin: Admin, password: str) -> EmailTemplate:
    subject = "Welcome to the Navabe Management Portal"
    text = (
        f"Hello {admin.first_name},\n\n"
        "Your Navabe management account is ready.\n\n"
    ...
    html = _layout(
        ...
        paragraphs=(
            "Your Navabe management account has been created.",
...
def admin_recovery_email(admin: Admin, password: str) -> EmailTemplate:
    subject = "Navabe management account recovery"
    text = (
        f"Hello {admin.first_name},\n\n"
        "A temporary password was generated for your Navabe management account.\n\n"
        f"Temporary password: {password}\n\n"
    ...
    html = _layout(
        ...
        greeting=f"Hello {admin.first_name},",
        paragraphs=(
            "Use the temporary password below to sign in to the Navabe management portal.",
...
<html lang="en">
  ...
  <body style="margin:0;padding:0;background:#f4f7fb;font-family:Arial,Helvetica,sans-serif;">
    ...
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#f4f7fb;margin:0;padding:0;">
      <tr>
        <td align="center" style="padding:32px 16px;">
          <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:640px;background:#ffffff;border:1px solid #dce5ef;border-radius:8px;overflow:hidden;">
            <tr>
              <td style="background:#0f172a;padding:24px 28px;">
                <table role="presentation" width="100%" cellpadding="0" cellspacing="0">
                  <tr>
                    <td style="vertical-align:middle;">
                      <span style="display:inline-block;width:40px;height:40px;border-radius:8px;background:#f8fafc;color:#0f172a;font-size:22px;font-weight:700;line-height:40px;text-align:center;">N</span>
                      <span style="display:inline-block;margin-left:12px;color:#ffffff;font-size:22px;font-weight:700;line-height:40px;vertical-align:top;">Navabe</span>
                    </td>
                  </tr>
                </table>
              </td>
            ...
            <tr>
              <td style="padding:32px 28px 12px;">
                {status_html}
                <h1 style="margin:0 0 18px;color:#0f172a;font-size:28px;line-height:1.25;font-weight:700;">{escaped_title}</h1>
                <p style="margin:0 0 16px;color:#0f172a;font-size:16px;line-height:1.55;">{escape(greeting)}</p>
                {paragraph_html}
                {details_html}
              </td>
            </tr>
            <tr>
              <td style="padding:4px 28px 32px;">
                <p style="margin:0;color:#64748b;font-size:14px;line-height:1.5;">{escape(footer_note)}</p>
              </td>
            </tr>
            <tr>
              <td style="background:#f8fafc;border-top:1px solid #e2e8f0;padding:18px 28px;">
                <p style="margin:0;color:#64748b;font-size:13px;line-height:1.5;">Navabe Bookstore<br>Catalog, customer accounts and management portal</p>
              </td>
            </tr>
          </table>
        </td>
...
def _status_badge(status: str) -> str:
    return (
        ...
        'border-radius:8px;background:#e0f2fe;color:#075985;font-size:13px;'
...
def _details_table(
    ...
    for label, value in details:
        ...
        rows.append(
            ...
            '<p style="margin:0 0 4px;color:#64748b;font-size:13px;line-height:1.35;">'
            f"{escape(label)}</p>"
            f'<p style="margin:0;color:#0f172a;line-height:1.35;{value_style}">'
            f"{escape(value)}</p>"
            "</td>"
            "</tr>"
        )
    ...
    return (
        '<table role="presentation" width="100%" cellpadding="0" cellspacing="0" '
        'style="margin:22px 0 18px;background:#f8fafc;border:1px solid #dce5ef;'
